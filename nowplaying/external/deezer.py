...
def get_client(include_cookies: bool = True) -> AsyncClient:
    cookies: dict[str, str] | None = {'arl': config.DEEZER_ARL_COOKIE, 'comeback': '1'}
    if not include_cookies:
        cookies = None

    user_agent: str = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) '
    user_agent += 'Chrome/68.0.3440.106 Safari/537.36'
...
class ScriptExtractor(html.parser.HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        self.scripts = []  # type: ignore[var-annotated]
        self.curtag = None
...
def genurlkey(songid, md5origin, mediaver=4, fmt=1):
    ...
    data_concat = b'\xa4'.join(
        _ for _ in [md5origin.encode(), str(fmt).encode(), str(songid).encode(), str(mediaver).encode()]
    )
...
def decryptfile(fh, key, fo) -> None:
    """
...
def get_picture_link(pic_idid):
    ...
    return setting_domain_img + '/cover/' + pic_idid + '/1200x1200.jpg'


async def writeid3v2(fo, song) -> None:
    def make28bit(x):
...
async def download_song(song: dict, output_file: BytesIO) -> bool:
    ...
    song_quality = (
        3
        if song.get('FILESIZE_MP3_320') and song.get('FILESIZE_MP3_320') != '0'
        else 5
        if song.get('FILESIZE_MP3_256') and song.get('FILESIZE_MP3_256') != '0'
        else 1
    )

    urlkey = genurlkey(song['SNG_ID'], song['MD5_ORIGIN'], song['MEDIA_VERSION'], song_quality)
    key = calcbfkey(song['SNG_ID'])
    url = 'https://e-cdns-proxy-{}.dzcdn.net/mobile/1/{}'.format(song['MD5_ORIGIN'][0], urlkey.decode())
...
async def get_song_infos_from_deezer_website(search_type, id):
    ...
    url = f'https://www.deezer.com/de/{search_type}/{id}'

    async with get_client() as session:
        resp = await session.get(url)

    if resp.status_code == 404:
        msg = f'ERROR: Got a 404 for {url} from Deezer'
        raise Deezer404Exception(msg)
    if 'MD5_ORIGIN' not in resp.text:
        msg = 'ERROR: we are not logged in on deezer.com. Please update the cookie'
        raise Deezer403Exception(msg)
    ...
    for script in parser.scripts:
        ...
        if regex:
            ...
            if DZR_APP_STATE['DATA']['__TYPE__'] == 'playlist' or DZR_APP_STATE['DATA']['__TYPE__'] == 'album':
                ...
                songs.extend(DZR_APP_STATE['SONGS']['data'])
