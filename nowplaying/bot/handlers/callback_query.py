...
from nowplaying.bot.handlers.buttons.toggle import handle_toggle_button
...
async def controls_handler(query: CallbackQuery) -> None:
    ...
    if query.data.startswith(CallbackButton.CONFIG_TOGGLE_PREFIX):
        await handle_toggle_button(query)
        return
